...
def run_backward(outputs:torch.Tensor, grad_tensor:torch.Tensor=None, input_tensor:torch.Tensor=None,model: nn.Module = None)->torch.Tensor:
    """
    Redefine backward pass
    Args: 
        outputs: Tensor,  layer's output tensor
          grad_tensor: Tensor, next-layer's gradient 
          input_tensor: Tensor, layer's input tensor
          optimizer, Optimizer, optimizer
          model: nn.Module, model (optional, used for device management)
...
##########################################################################梯度裁剪
    torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)  # 限制梯度范数最大值为1.0
...
class Server_Model(nn.Module):
    ...
    def forward(self, hidden_states: torch.LongTensor = None, label_ids: torch.LongTensor = None):
        ...
        if label_ids is not None:
            ...
            # 如果没有 label_ids，使用全零张量作为占位符
            inputs_embeds = torch.zeros((batch_size, max_length, 4096), device=hidden_states.device)
        #############################################################################################
        # # wte 
        # inputs_embeds = self.embed_tokens(label_ids)
...
########################################################################################################################
    ...
    # model = LlamaModel.from_pretrained(config.model_name).to(device)
    model = DeepSeekModel.from_pretrained(config.model_name, quantization_config=config.bnb_config, device_map=device)
########################################################################################################################
    
    # Load Llama2 state_dict
    # if isinstance(config.state_dict, str):
    #     print(config.state_dict)
    #     state_dict = torch.load(config.state_dict, weights_only=True,map_location=torch.device("cpu"))
