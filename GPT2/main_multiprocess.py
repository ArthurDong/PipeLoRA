...
def setup_gpt_models(args):
    ...
    input_ids = torch.randint(0, 50257, (args.max_length,))
    position_ids = torch.tensor(range(args.max_length), dtype=torch.long)
    dummy_testcase = (input_ids, position_ids, input_ids)
    writer.add_graph(model, dummy_testcase)

    # convert GPT model to LoRA-GPT
    LoRA_GPT2(gpt_model=model, r=4, alpha=8, lora_layer=args.lora_layers)
...
def train_client(args, model, dataloader, writer:SummaryWriter):
    ...
    with open('results/result.txt', 'a+') as f:
        f.write("Cl: batch_size:%d, num_split:%d, fw_avg:%.2f, bw_avg:%.2f \n"
                %(args.batch_size, args.num_split, train_fw_time*50/global_step, train_bw_time*50/global_step))
...
def train_server(args, model, dataloader, writer:SummaryWriter):
    ...
    # vram = [torch.cuda.memory_allocated() / (args.max_length ** 2)]  # MB
    ...
    for batch in trainloader:
        ...
        with open('results/convergence.txt', 'a+') as f:
            f.write(f"num_split:{args.num_split:d}, lora_layers:{args.lora_layers}, loss:{total_loss/global_step:.3f} \n")

    with open('results/result.txt', 'a+') as f:
        f.write("Sv: batch_size:%d, num_split:%d, loss:%f, total_time:%.2f, fw_avg:%.2f, bw_avg:%.2f \n"
                %(args.batch_size, args.num_split, total_loss/global_step, trainloader.format_dict['elapsed'], train_fw_time*50/global_step, train_bw_time*50/global_step))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--lora_layers", type=str, default='attn_mlp', choices=["attn", "mlp", "ln", "attn_mlp", "attn_ln", "mlp_ln", "attn_mlp_ln"])
