...
def pred_text(pred_text, models):
    ...
    models['model_cl'].to(args.device_cl)
...
def train_client(args, model, dataloader, writer:SummaryWriter):
    ...
    backward_grad = torch.empty([args.batch_size,1024,768]).to(args.device_cl)
...
def train_server(args, model, dataloader, writer:SummaryWriter):
    ...
    trainloader, testloader = dataloader
    forward_feature = torch.empty([args.batch_size,1024,768]).to(args.device_sv)
